...
build() {
  ...
  if [ -d /root/tools/centminmod-digitalocean-marketplace/packer ]; then
    ...
    echo "time TMPDIR=/home/packertmp PACKER_LOG=1 packer build -var "do_image=$do_imageid" -var "do_size=$do_size" -var "enable_argon=y" -var 'enable_phppgo=y' -var 'install_redis=y' -var 'enable_phpfpm_systemd=y' packer-centos7-basic.json"
    time TMPDIR=/home/packertmp PACKER_LOG=1 packer build -var "do_image=$do_imageid" -var "do_size=$do_size" -var "enable_argon=y" -var 'enable_phppgo=y' -var 'install_redis=y' -var 'enable_phpfpm_systemd=y' packer-centos7-basic.json
