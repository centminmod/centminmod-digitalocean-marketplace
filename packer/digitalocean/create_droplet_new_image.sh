#!/bin/bash
########################################################
# create droplet from snapshot image id & then create 
# new snapshot images
########################################################
...
# shutdown
curl -sX POST -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" -d '{"type":"shutdown"}' "https://api.digitalocean.com/v2/droplets/${droplet_id}/actions" | jq -r .

# new image
snapshot_count=1
for c in $(seq 1 $snapshot_count); do
  snapshot_new_name="${droplet_image_id}-created_snapshot${snapshot_count}_${DT}"
  curl -sX POST -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" -d "{\"type\":\"snapshot\",\"name\":\"${snapshot_new_name}\"}" "https://api.digitalocean.com/v2/droplets/${droplet_id}/actions" | jq -r .
done
...
do_template
do_create
} 2>&1 | tee "created-extra-snapshots-${DROPLET_DATAFILE}"