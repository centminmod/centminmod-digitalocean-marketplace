...
build() {
  ...
  if [ -d /root/tools/centminmod-digitalocean-marketplace/packer ]; then
    ...
    # validate
    echo
    echo "packer validate packer-centos7-only.json"
    packer validate packer-centos7-only.json

    # inspect
    echo
    echo "packer inspect packer-centos7-only.json"
    packer inspect packer-centos7-only.json

    # build
    echo
    snapshot_new_name="centos7-latest-only-packer-${dt}"
    snapshot_new_name_second="centos7-latest-only-packer-2-${dt}"
    export PACKER_LOG_PATH="centos7-latest-only-packer-$(date +"%d%m%y-%H%M%S").log"
    echo "time TMPDIR=/home/packertmp PACKER_LOG=1 packer build packer-centos7-only.json"
